...
const CMS_DATA = {
  ...
  // Site Settings
  siteSettings: {
    practiceName: "Blanchard Orthodontics",
    tagline: "Your Smile. Our Passion.",
    phone: "(903) 707-6275",
    email: "info@blanchardorthodontics.com",
    address: "Jacksonville & Tyler, Texas",
    socialMedia: {
      facebook: "https://www.facebook.com/profile.php?id=61552167444126",
      instagram: "https://www.instagram.com/blanchardorthoetx/"
    },
    officeHours: {
...
// Helper functions for CMS functionality
const CMS = {
  // Get active info banner
  getActiveInfoBanner() {
    return CMS_DATA.infoBanners.find(banner => banner.isActive);
  },

  // Get all published articles
  getArticles() {
    return CMS_DATA.articles.filter(article => article.isPublished);
  },

  // Get article by slug
  getArticleBySlug(slug) {
    return CMS_DATA.articles.find(article => article.slug === slug && article.isPublished);
  },

  // Get articles by category
  getArticlesByCategory(categorySlug) {
    return CMS_DATA.articles.filter(article => 
  ...
  // Get all categories
  getCategories() {
    return CMS_DATA.categories;
  },

  // Get category by slug
  getCategoryBySlug(slug) {
    return CMS_DATA.categories.find(category => category.slug === slug);
  },

  // Get all active team members
  getTeamMembers() {
    return CMS_DATA.teamMembers.filter(member => member.isActive);
  },

  // Get team member by slug
  getTeamMemberBySlug(slug) {
    return CMS_DATA.teamMembers.find(member => member.slug === slug && member.isActive);
  },
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CMS_DATA, CMS };
}

// Make CMS available globally in browser
